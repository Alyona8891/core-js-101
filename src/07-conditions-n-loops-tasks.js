...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (right1 > rect2.left && bottom1 > rect2.top) {
    return true;
  }
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  const result = [];
  arr.forEach((el, i) => {
    const sliceArr = arr.slice(0, i).concat(arr.slice(i + 1));
...
function isCreditCardNumber(ccn) {
  ...
  let str = '';
  let result = 0;
  for (let i = 0; i < ccn.toString().length - 1; i += 1) {
    arr.push(+(ccn.toString()[i]));
  }
  ...
  const res = 10 * Math.ceil(result / 10) - result;
  if (res === +(ccn.toString()[ccn.toString().length - 1])) {
    return true;
  }
  return false;
}
...
function getDigitalRoot(num) {
  ...
  const res = arr.reduce((acc, curVal) => acc + curVal, 0);
  if (res > 10) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (closedBrackets.includes(symbol)) {
      const index = closedBrackets.indexOf(symbol);
      if (stack.length > 0 && stack[stack.length - 1] === openedBrackets[index]) {
        stack.pop();
      } else if (stack.length === 0) {
        return false;
      }
    } else if (openedBrackets.includes(symbol)) {
      stack.push(symbol);
    }
  }
...
function toNaryString(num, n) {
  let result = '';
  const newNun = Math.floor(num / n);
  const remains = num - newNun * n;
  result = remains + result;
  if (newNun !== 0) {
    result = toNaryString(newNun, n) + result;
  }
  return result;
}
...
function getCommonDirectoryPath(pathes) {
  const pathesArrs = pathes.map((el) => el.split('/'));
  const lengthArrs = pathesArrs.map((el) => el.length);
  const minLength = Math.min(...lengthArrs);
  let result = [];
  const set = new Set();
  for (let j = 0; j < minLength; j += 1) {
    for (let i = 0; i < pathesArrs.length; i += 1) {
      set.add(pathesArrs[i][j]);
    }
    if (set.size === 1) {
      result = result.concat(Array.from(set));
      set.clear();
    }
  }
  if (result.length === 0) {
...
function getMatrixProduct(m1, m2) {
  const lengthM1 = m1.length;
  const lengthM2 = m2.length;
  const result = [];
  for (let i = 0; i < lengthM1; i += 1) {
    result.push([]);
  }
  for (let x = 0; x < m2[0].length; x += 1) {
    for (let i = 0; i < lengthM1; i += 1) {
      let el = 0;
      for (let j = 0; j < lengthM2; j += 1) {
        el += m1[i][j] * m2[j][x];
      }
      result[i][x] = el;
    }
